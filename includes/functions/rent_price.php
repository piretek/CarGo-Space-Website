...
function rent_price( $id, $isCar = false, $from = null, $to = null ) {
  ...
  if (!$isCar) {
    $query = sprintf("SELECT rents.id, rents.begin, rents.end, rents.insurance, IF(cars.price = 0.00, models.price, cars.price) as price FROM (rents INNER JOIN cars ON rents.car = cars.id) INNER JOIN models ON cars.model = models.id WHERE rents.id = '%s'",
  ...
  if ($rents->num_rows == 0) {
    ...
    if (array_key_exists('begin', $rent) && array_key_exists('end', $rent)) {
      ...
      if (array_key_exists('insurance', $rent) && $rent['insurance'] == 1) {
        $price = $price + ((round(abs($rent['end'] - $rent['begin']) / 60 / 60 / 24, 2) - 1) * 39.90);
      }
    }
      ...
      if (array_key_exists('insurance', $rent) && $rent['insurance'] == 1) {
        $price = $price + ((abs($from - $to) / 60 / 60 / 24 ) * 39.90);
      }
    }
    else {
